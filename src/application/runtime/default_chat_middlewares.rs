...
use crate::ports::outbound::ai_chat_client::{StreamDelta, send_stream_delta};
...
impl AiChatClient for LoggingChatClient {
    ...
    async fn complete_stream(
        ...
        let started = Instant::now();
        eprintln!(
            "stasis.chat stream request messages={} options_present={}",
            request.messages.len(),
            options.is_some()
        );

        match self.inner.complete_stream(request, options, chunk_tx).await {
            Ok(response) => {
                eprintln!(
                    "stasis.chat stream response ok elapsed_ms={}",
                    started.elapsed().as_millis()
                );
                Ok(response)
            }
            Err(err) => {
                eprintln!(
                    "stasis.chat stream response error elapsed_ms={} error={}",
                    started.elapsed().as_millis(),
                    err
                );
                Err(err)
...
impl AiChatClient for CacheChatClient {
    ...
    async fn complete_stream(
        ...
        if let Some(cached) = self.cache.get(&cache_key) {
            ...
            if let (Some(tx), Some(text)) = (chunk_tx, cached.first_text()) {
