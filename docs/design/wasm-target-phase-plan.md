...
`--no-default-features` on native is a slim kernel (no dashboard/genai/grapheme/surreal-kv). That is intentional.
...
| ADR | 0009 Accepted when W1 merged |
...
**Status (2026-09-17):** Delivered. `default = ["native"]` splits dashboard, Surreal, genai, Grapheme, and env-fs; Tokio is target-specific (`rt` on wasm32); `.github/workflows/wasm.yml` gates the check.
