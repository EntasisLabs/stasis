...
| WASM + Grapheme guest | `cargo check -p stasis-rs --target wasm32-unknown-unknown --no-default-features --features llm-openai-http,grapheme` | Tool-loop + `grapheme-wasm` 0.7.1 in-guest engine |
| WASM npm package | `npm --prefix stasis-wasm run build && npm --prefix stasis-wasm test && npm --prefix stasis-wasm run pack:dry` | Node/bundler package with TypeScript types (`stasis-wasm@0.12.0`; publish with `./scripts/publish-npm.sh`) |

WASM does **not** include `stasisd`, the Axum dashboard, SurrealKV, genai's native TLS client, or the Grapheme **host** SDK. Enable `grapheme` (without `grapheme-host`) to run workflows through `grapheme-wasm` 0.7.1. Inject `LlmGateway` (`MockLlmGateway` or `OpenAiHttpGateway`) and use `RuntimeBackend::InMemory` (or `RuntimeSdk::surreal_ws` with `--features surreal-ws`). See [ADR-0009](docs/adr/ADR-0009-wasm-target-profile.md) and [the WASM phase plan](docs/design/wasm-target-phase-plan.md). Browser cookbook: [embed Stasis in a browser host](docs-book/src/cookbook/embed-stasis-browser-host.md).
...
- Inbound job triggers (`accept_inbound_json` / `accept_inbound_job`) so a webhook, TCP line, Kafka record, or RabbitMQ delivery can enqueue a job. Duplicate idempotency keys return the original job.
